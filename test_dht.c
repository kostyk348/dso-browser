...
static void test_announce_lookup(void) {
    ...
    assert(n == 1);
    assert(ips[0] == inet_addr("192.168.1.7"));
    assert(ports[0] == 7000);

    /* Unknown hash -> 0 holders */
    assert(dht_lookup(&dht, 0xDEAD, ips, ports) == 0);
    PASS();
}
...
static void test_record_propagate(void) {
    ...
    assert(n == 1);
    assert(ips[0] == inet_addr("1.2.3.4"));
    PASS();
}
...
static void test_ed25519_sign_verify(void) {
    ...
    assert(psirp_verify(kp.public_key, (const uint8_t *)msg, strlen(msg), sig));
    PASS();
}
...
static void test_ed25519_wrong_key(void) {
    ...
    assert(!psirp_verify(b.public_key, (const uint8_t *)msg, strlen(msg), sig));
    PASS();
}
...
static void test_signed_content_roundtrip(void) {
    ...
    assert(psirp_sign_content(&kp, payload, sizeof(payload) - 1, buf, &slen));
    size_t clen = 0;
    const uint8_t *out = psirp_verify_content(buf, slen, &clen);
    assert(out && clen == sizeof(payload) - 1);
    assert(memcmp(out, payload, clen) == 0);
    PASS();
}
